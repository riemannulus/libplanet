        ...
        public IReadOnlyList<IActionResult> Evaluate(IPreEvaluationBlock block)
        {
            ...
            try
            {
                ...
                else
                {
                    if (evaluations.Any())
                    {
                        previousState = new Account(
                            _blockChainStates.StateStore.GetStateRoot(
                                evaluations
                                    .Last()
                                    .OutputRootHash),
                            new AccountDelta(),
            ...
            ILogger? logger = null)
        {
            ...
            foreach (IAction action in actions)
            {
                ...
                (ActionEvaluation Evaluation, long NextGasLimit) result = EvaluateAction(
                    ...
                    action,
                    logger);
                ITrie trie = blockChainStates.StateStore.Commit(result.Evaluation.OutputState.Trie);
                ActionEvaluation evaluation =
                    new ActionEvaluation(
                        ...
                        new Account(
                            ...
                            Account.GetTotalUpdatedFungibles(result.Evaluation.OutputState)),
